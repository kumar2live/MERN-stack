...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError('Due to technical error', 500)
    );
  }
  ...
  const createUser = new User({
    name,
    email,
    image: 'https://images.pexels.com/photos/753339/pexels-photo-753339.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
    password,
    places: [],
  });
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError('Due to technical error Logging In failed', 500)
    );
  }
...
const getUsers = async (req, res, next) => {
  ...
  if (!users || users.length === 0) {
    return next(
      new HttpError('Could not any users!', 404),
    );
  } 

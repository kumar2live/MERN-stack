...
const getAllPlaces = async (req, res, next) => {
  ...
  try {
    ...
    const getError = new HttpError(
      "Could not find a place due to tech error",
      404
    );
  ...
  if (!places || places.length === 0) {
    return next(new HttpError("Could not any places!", 404));
  }
...
const getPlaceById = async (req, res, next) => {
  ...
  if (!place) {
    const placeError = new HttpError(
      "Could not find a place for given ID",
      404
    );
...
const deletePlaceById = async (req, res, next) => {
  ...
  if (!place) {
    return next(
      new HttpError(
        "Unable to find the place that you are trying to delete",
        500
      )
    );
  }
